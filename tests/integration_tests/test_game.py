...
async def queue_player_for_matchmaking(user, lobby_server, queue_name="ladder1v1"):
    async with connect_and_sign_in(user, lobby_server) as (player_id, _, proto):
        await read_until_command(proto, "game_info")
        await start_search(proto, queue_name)
        yield player_id, proto


@contextlib.asynccontextmanager
async def queue_players_for_matchmaking(lobby_server, queue_name: str = "ladder1v1"):
    async with queue_player_for_matchmaking(
            ("ladder1", "ladder1"),
            lobby_server,
            queue_name
    ) as (player1_id, proto1), connect_and_sign_in(
        ("ladder2", "ladder2"),
        lobby_server
    ) as (player2_id, _, proto2):
        ...
        yield player1_id, proto1, player2_id, proto2
...
async def queue_temp_players_for_matchmaking(
    ...
    async with AsyncExitStack() as stack:
        ...
        protos = [
            await stack.enter_async_context(queue_player_for_matchmaking(user, lobby_server, queue_name))
            for user in users
        ]
...
async def test_game_join_not_ready(lobby_server):
    async with connect_and_sign_in(
            ("test", "test_password"), lobby_server
    ) as (_, _, host_proto), connect_and_sign_in(
...
async def test_game_info_messages(lobby_server):
    async with connect_and_sign_in(
            ("test", "test_password"), lobby_server
    ) as (host_id, _, host_proto), connect_and_sign_in(
        ("Rhiza", "puff_the_magic_dragon"), lobby_server
    ) as (guest_id, _, guest_proto):
        await read_until_command(guest_proto, "game_info")
        await read_until_command(host_proto, "game_info")
...
async def test_restore_game_session_live_wrong_game(lobby_server):
    async with connect_and_sign_in(
        ...
        ("Rhiza", "puff_the_magic_dragon"), lobby_server
    ) as (guest_id, _, guest_proto), connect_and_sign_in(
    ...
    async with connect_and_sign_in(
        ...
        async with AsyncExitStack() as stack:
            ...
            for _ in range(3):
                ...
                guest_id, _, proto = await stack.enter_async_context(connect_and_sign_in(
                    user, lobby_server
                ))
...
async def test_ladder_game_draw_bug(lobby_server, database):
    ...
    instead of a draw.
    """
    async with queue_players_for_matchmaking(lobby_server) as (player1_id, proto1, player2_id, proto2):
...
async def test_ladder_game_not_joinable(lobby_server):
    ...
    async with connect_and_sign_in(
            ("test", "test_password"), lobby_server
    ) as (_, _, test_proto), queue_players_for_matchmaking(lobby_server) as (_, proto1, _, _):
...
async def test_gamestate_ended_clears_references(
    ...
    async with connect_and_sign_in(
            ("test", "test_password"), lobby_server
    ) as (test_id, _, test_proto), connect_and_sign_in(
        ("Rhiza", "puff_the_magic_dragon"), lobby_server
    ) as (rhiza_id, _, rhiza_proto):
        await read_until_command(test_proto, "game_info")
...
async def test_galactic_war_2v1_game_ended_broadcasts_army_results(lobby_server, channel):
    ...
    ) as (other_guest_id, _, other_guest_proto):
        await read_until_command(guest_proto, "game_info")
