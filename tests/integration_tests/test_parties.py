...
async def test_invite_party_workflow(lobby_server, party_service):
    ...
    ) as (test_id, _, proto), connect_and_sign_in(
...
async def test_player_switches_parties(lobby_server):
    ...
    async with connect_and_sign_in(
        ...
        ("rhiza", "puff_the_magic_dragon"), lobby_server
    ) as (rhiza_id, _, proto2), connect_and_sign_in(
        ("test2", "test2"), lobby_server
    ) as (test_2_id, _, proto3):
        await read_until_command(proto, "game_info")
        await read_until_command(proto2, "game_info")
...
async def test_accept_while_party_queued(lobby_server):
    async with connect_and_sign_in(
            ("test", "test_password"), lobby_server
    ) as (test_id, _, proto1), connect_and_sign_in(
        ("rhiza", "puff_the_magic_dragon"), lobby_server
    ) as (rhiza_id, _, proto2):
...
async def test_join_party_after_disband(lobby_server):
    ...
    ) as (p1_id, _, proto), connect_and_sign_in(
