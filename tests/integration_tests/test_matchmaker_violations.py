...
async def test_violation_for_guest_timeout(mocker, lobby_server):
    ...
    async with queue_players_for_matchmaking(lobby_server) as (_, host, guest_id, guest):
...
async def test_violation_persisted_across_logins(mocker, lobby_server):
    ...
    async with queue_players_for_matchmaking(lobby_server) as (host_id, host, _, guest):
...
async def test_violation_config_disabled(mocker, monkeypatch, lobby_server):
    ...
    async with queue_players_for_matchmaking(lobby_server) as (_, host, _, guest):
