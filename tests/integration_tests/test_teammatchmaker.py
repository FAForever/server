...
async def connect_players(lobby_server):
    async with AsyncExitStack() as stack:
        res = [
            await stack.enter_async_context(connect_and_sign_in(
                (f"ladder{i + 1}",) * 2, lobby_server
            ))
            for i in range(4)
        ]
...
async def queue_players_for_matchmaking(lobby_server):
    async with connect_players(lobby_server) as (protos, ids):
...
async def test_game_matchmaking(lobby_server):
    async with queue_players_for_matchmaking(lobby_server) as (protos, _):
...
async def test_game_matchmaking_multiqueue(lobby_server):
    async with connect_players(lobby_server) as (protos, _):
        await asyncio.gather(*[
            read_until_command(proto, "game_info") for proto in protos
        ])
...
async def test_newbie_matchmaking_with_parties(lobby_server):
    ...
    async with connect_and_sign_in(
            ("ladder1", "ladder1"), lobby_server
    ) as (id1, _, proto1), connect_and_sign_in(
        ("ladder2", "ladder2"), lobby_server
    ) as (id2, _, proto2), connect_and_sign_in(
        ("tmm1", "tmm1"), lobby_server
    ) as (_, _, proto3), connect_and_sign_in(
...
async def test_game_ratings(lobby_server):
    async with queue_players_for_matchmaking(lobby_server) as (protos, ids):
...
async def test_ratings_initialized_based_on_global_persisted(
    ...
    async with connect_and_sign_in(
            ("ladder1", "ladder1"), lobby_server
    ) as (player1_id, _, proto1), connect_and_sign_in(
        ("ladder2", "ladder2"), lobby_server
    ) as (player2_id, _, proto2), connect_and_sign_in(
        ("test", "test_password"), lobby_server
    ) as (player3_id, _, proto3), connect_and_sign_in(
        ("tmm2", "tmm2"), lobby_server
    ) as (player4_id, _, proto4):
        protos = [proto1, proto2, proto3, proto4]
        for proto in protos:
