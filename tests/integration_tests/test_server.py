...
async def test_server_proxy_mode_direct(lobby_server_proxy, caplog):
    with caplog.at_level("TRACE"):
        async with connect_client(lobby_server_proxy) as proto:
...
async def test_very_long_message(lobby_server, caplog):
    async with connect_and_sign_in(("test", "test_password"), lobby_server) as (_, _, proto):
        DATA_SIZE = 2 ** 20
...
async def test_graceful_shutdown(
    ...
    async with connect_and_sign_in(
            await tmp_user("Player"), lobby_server
    ) as (_, _, proto):
        await read_until_command(proto, "game_info")
...
async def test_graceful_shutdown_kick(
    ...
    async with connect_and_sign_in(
            await tmp_user("Idle"), lobby_server
    ) as (_, _, idle_proto), connect_and_sign_in(
        await tmp_user("Host"), lobby_server
    ) as (_, _, host_proto), connect_and_sign_in(
        await tmp_user("Player"), lobby_server
    ) as (player1_id, _, player1_proto), connect_and_sign_in(
        await tmp_user("Player"), lobby_server
    ) as (player2_id, _, player2_proto):
        protos = (
            idle_proto,
            host_proto,
            player1_proto,
...
async def test_player_info_broadcast(lobby_server):
    async with connect_client(lobby_server) as p2:
        async with connect_client(lobby_server) as p1:
...
async def test_info_broadcast_authenticated(lobby_server):
    async with connect_client(lobby_server) as proto1, connect_client(lobby_server) as proto2, connect_client(
            lobby_server) as proto3:
...
async def test_game_info_broadcast_to_players_in_lobby(lobby_server):
    ...
    async with connect_and_sign_in(
            ("friends", "friends"), lobby_server
    ) as (friends_id, _, proto1), connect_and_sign_in(
        ("test", "test_password"), lobby_server
    ) as (test_id, _, proto2):
        await read_until_command(proto1, "game_info")
        await read_until_command(proto2, "game_info")
...
async def test_game_host_authenticated(lobby_server, user):
    async with connect_and_sign_in(user, lobby_server) as (_, _, proto):
        await read_until_command(proto, "game_info")
...
async def test_avatar_select(lobby_server, database):
    ...
    async with connect_and_sign_in(
            ("player_service1", "player_service1"), lobby_server
    ) as (player_id, _, proto):
        await read_until_command(proto, "game_info")
