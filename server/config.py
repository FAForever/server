...
class ConfigurationStore:
    ...
    def __init__(self):
        ...
        # Number of recent games in the queue per player to check for anti-repetition (within last 24h)
        # Higher values make antirep system eliminate more maps from the pool
        # which counteracts player vetoes and may lead to worse user experience if pool is not big enough
        self.LADDER_ANTI_REPETITION_LIMIT = 1
        # Controls how strictly played maps are replaced with unplayed ones
        # Higher values = more restrictive, only replaces with similarly-popular maps
        # Lower values = less restrictive, allows more varied replacements
        self.LADDER_ANTI_REPETITION_WEIGHT_BASE_THRESHOLDS = [0.75, 0.5]
        # How strongly to avoid maps played multiple times / by multiple players
        # Higher (closer to 1) = weaker avoidance
        # Lower (closer to 0) = stronger avoidance
